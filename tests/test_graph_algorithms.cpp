...
TEST(NaiveSingle, TerminatesOnSmallGraph) {
    ...
    const auto result = graph_structured::simulate_naive_single(g, 1.5, 0, rng, 100'000);
...
TEST(NaiveMC, CompleteGraphMatchesWellMixed) {
    ...
    const auto exact_val = exact::well_mixed(N, r);

    const SimulationConfig config{
        .accuracy = {.epsilon = 0.05, .delta = 0.25},
        .seed = 42, .num_threads = 1};

    const auto g = make_complete_graph<double>(N);
    const auto result = graph_structured::naive_mc_fixation(g, r, config);
...
TEST(NaiveMC, CycleIsothermal) {
    ...
    const auto g = make_cycle_graph<double>(N);
    const auto result = graph_structured::naive_mc_fixation(g, r, config);
    ASSERT_TRUE(result.has_value());
    EXPECT_NEAR(result->estimate, exact_val, 0.05);
}
...
TEST(NaiveMC, NeutralDrift) {
    ...
    const auto g = make_complete_graph<double>(N);
    const auto result = graph_structured::naive_mc_fixation(g, 1.0, config);
    ASSERT_TRUE(result.has_value());
...
TEST(StarGraph, AmplifiesSelection) {
    ...
    const auto star = make_star_graph<double>(N);
    const auto result = graph_structured::naive_mc_fixation(star, r, config);
    ASSERT_TRUE(result.has_value());
...
TEST(Multithreaded, ConsistentResults) {
    ...
    const auto result_1t = graph_structured::naive_mc_fixation(g, r, config_1t);
    const auto result_mt = graph_structured::naive_mc_fixation(g, r, config_mt);
...
TEST(StepsTotal, NaiveMCPopulated) {
    const auto g = make_complete_graph<double>(8);
    const SimulationConfig config{
        .accuracy = {.epsilon = 0.1, .delta = 0.25},
        .seed = 42, .num_threads = 1};
    const auto result = graph_structured::naive_mc_fixation(g, 1.5, config);
    ASSERT_TRUE(result.has_value());
