...
TEST(FixationProbability, SingleIndividual) {
    auto result = fixation_exact<double>(1, 2.0);
    ASSERT_TRUE(result.has_value());
...
TEST(FixationGeneral, ConstantFitnessMatchesExact) {
    ...
    auto exact = fixation_exact<double>(N, r);
    auto general = fixation_general<double>(N, 1,
...
TEST(FixationFrom, BoundaryConditions) {
    auto r0 = fixation_from<double>(10, 0, 1.5);
    ASSERT_TRUE(r0.has_value());
    EXPECT_DOUBLE_EQ(*r0, 0.0);

    auto rN = fixation_from<double>(10, 10, 1.5);
    ASSERT_TRUE(rN.has_value());
    EXPECT_DOUBLE_EQ(*rN, 1.0);

    auto r_too_big = fixation_from<double>(10, 11, 1.5);
