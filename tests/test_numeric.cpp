...
TEST(KahanSummation, SpanOverload) {
    const std::vector<double> vals = {1e15, 1.0, -1e15, 1.0};
...
TEST(LogSumExp, SimpleCase) {
    const std::vector<double> vals = {std::log(1.0), std::numbers::ln2, std::log(3.0)};
    // log(1 + 2 + 3) = log(6)
...
TEST(LogSumExp, LargeValues) {
    ...
    const std::vector<double> vals = {1000.0, 1000.0 + std::numbers::ln2};
...
TEST(LogSumExp, NegativeInfinity) {
    const std::vector<double> vals = {-std::numeric_limits<double>::infinity()};
...
TEST(FixationFromLogGammas, NeutralDrift) {
    ...
    const std::vector<double> log_gamma(N - 1, 0.0);
...
TEST(FixationFromLogGammas, AdvantageousMutant) {
    ...
    const std::vector<double> log_gamma(N - 1, std::log(1.0 / r));
    const auto phi = fixation_from_log_gammas<double>(
        std::span<const double>(log_gamma));
